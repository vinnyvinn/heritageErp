...
//include_once("../../includes/references.inc");
...
//	{
//	    $input_error = 1;
//		display_error( _('The item code cannot be emptykkk'));
	...
	if ($input_error != 1)
	{
		...
		{ /*so its an existing one */
		      update_item($_POST['NewStockID'], $_POST['description'],$_POST['type_id'],get_type($_POST['type_id'])['name'],
				...
				check_value('no_sale'), check_value('editable'), check_value('no_purchase'),check_value('lot_item'),
...
function item_settings(&$stock_id, $new_item) 
{
	...
	if ($new_item) 
	{
        ...
        if (isset($_POST['variety_id'])){
            ...
            $color = get_color($_POST['color_id'])['name'];
	...
	if ($new_item && (list_updated('category_id') || !isset($_POST['sales_account']))) { // changed category for new item or first page view
		...
		$_POST['lot_item'] = $category_record["lot_item"];
		$_POST['editable'] = 0;
	}
	...
	// show inactive item tax type in selector only if already set.
     item_tax_types_list_row(_("Item Tax Type:"), 'tax_type_id', null, false);
	...
	if (!$new_item && $_POST['lot_item']){
        check_row_disabled(_("Lot Item:"), 'lot_item');
    }else {
        check_row(_("Lot Item:"), 'lot_item');
    }
    ...
    display_heading_("Lot Items cannot be changed.","lot_item");
	div_end();
