...
function edit_link($row) 
{
    ...
    return pager_link( _("View Invoice"),
        "/sales/view_invoice_entry.php?ViewOrderNumber=" .$row['order_no'], ICON_VIEW);
}

function dispatch_link($row)
{
      return pager_link( _("Edit Invoice"),
			"/sales/edit_invoice.php?OrderNumber=" .$row['order_no'], ICON_EDIT);
}
...
function order_link($row)
{
    if ($row['trans_type'] == ST_SALESQUOTE):
        if (isset($row['qty_sent']) && $row['qty_sent'] > 0){
            return '';
        }
    endif;
